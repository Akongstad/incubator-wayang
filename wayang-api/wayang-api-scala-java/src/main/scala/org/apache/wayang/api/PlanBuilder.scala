...
import org.apache.wayang.basic.operators.{CollectionSource, ObjectFileSource, TableSource, TextFileSource, ParquetSource, ParquetRecordSource}
...
class PlanBuilder(private[api] val wayangContext: WayangContext, private var jobName: String = null) {
  ...
  def readParquetFile(url: String): DataQuanta[String] = load(new ParquetSource(url))
  def readParquetRecordFile(url: String): DataQuanta[Record] = load(new ParquetRecordSource(url))
