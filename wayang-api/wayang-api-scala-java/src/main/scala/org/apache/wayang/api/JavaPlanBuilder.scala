...
import org.apache.wayang.basic.operators.{TableSource, TextFileSource, KafkaTopicSource, ParquetSource, ParquetRecordSource}
...
class JavaPlanBuilder(wayangCtx: WayangContext, jobName: String) {
  ...
  /**
    * Reads a parquet file and provides the data as a dataset of [[Record]]s.
    *
  ...
  /**
    * Reads a parquet file and provide it's dataset as [[Record]]s.
    *
    * @param source from that the [[Record]]s should be read
    * @return [[DataQuantaBuilder]] for the [[Record]]s in the table
    */
  def readParquetRecordFile(source: ParquetRecordSource) =
